...
public class Main {
  ...
  private static FacturacionEmpresa getVentaSemanalByClient(String key, Double valorVentaSemanal){
    return FacturacionEmpresa.builder().empresa(key).ventaMensual(valorVentaSemanal).build();
  }

  private static void agruparVentas() {
    //Observable<Integer> observable = Observable.range(0, 10);
    //observable.groupBy(v -> v%2 == 0).subscribe(group -> System.out.println(group.getKey()));

    List<FacturacionEmpresa> facturacionEmpresaList = getVentaSemanalAllClients();
    Observable.fromIterable(facturacionEmpresaList)
              .groupBy(factura -> factura.getEmpresa())
              .flatMap(grouped0 -> grouped0.buffer(4))
              .subscribe(s -> System.out.println(s));
    /*
        Observable.fromIterable(facturacionEmpresaList).groupBy(factura -> factura.getEmpresa())
        .flatMap(groupedO -> groupedO.buffer(10) // [2]
